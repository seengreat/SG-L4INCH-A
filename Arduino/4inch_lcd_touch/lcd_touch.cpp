...
{
    unsigned char i, j;
    unsigned int adc_buff[READ_TIMES];
...
*******************************************************************************/
static void TP_Read_ADC_XY(unsigned int *x_adc, unsigned int  *y_adc )
{
...
static bool TP_Read_ADC_XY2(unsigned int *x_adc, unsigned int  *y_adc )
{
...
static void TP_calib_info_show(uint16_t pos_xy[5][2], double px, double py)
{
    ...
    sbuf[1] = 'x';
    sbuf[2] = ':';
    dtostrf(px,1,2,sbuf+3);
    sbuf[7] = 0;//add end character
    Gui_draw_str(40, 160 + (i * 20), sbuf, &Font16, RED, BACKGROUND_COLOR);
    sbuf[0] = 'p';
    sbuf[1] = 'y';
    sbuf[2] = ':';
    dtostrf(py,1,2,sbuf+3);
    sbuf[7] = 0;//add end character
    Gui_draw_str(40 + 100, 160 + (i * 20), sbuf, &Font16, RED, BACKGROUND_COLOR);
}
     ...
     { 
        ...
        {        
            ...
            case 5:     //had got all 5 points 
              ...
              Serial.println("xfac:"+String(tp_dev.xfac)+"F"+"  "+"yfac:"+String(tp_dev.yfac)+"F");
              tp_dev.xc = pos_temp[4][0];      //X axis, physical center coordinates
              tp_dev.yc = pos_temp[4][1];      //Y axis, physical center coordinates
              LCD_Clear(WHITE);  //clear screen
              Gui_draw_str(35, 110, "Touch Screen Calibration OK!", &Font16, BLUE, BACKGROUND_COLOR);//Calibration completed
              delay(2000);
